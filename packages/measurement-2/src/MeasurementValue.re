...
let encode = (e: t) => {
  let n = typeToName(e);
  switch (e) {
  | `FloatPoint(k) => Utils.makeEncode(Json.Encode.float, n, k)
  | `FloatCdf(k) => FloatCdf.encode(n, k)
  | `Percentage(k) => Utils.makeEncode(Json.Encode.float, n, k)
  | `Binary(k) => Utils.makeEncode(Json.Encode.bool, n, k)
  | `UnresolvableResolution(_k) =>
    Utils.makeEncode(Json.Encode.string, n, stringOfValue(e))
  | `Comment(_k) => Utils.makeEncode(Json.Encode.string, n, stringOfValue(e))
  };
};
...
let decoder = (a, j: Js.Json.t): Belt.Result.t(t, string) =>
  switch (a) {
  | `FloatPoint =>
    j |> convert(Utils.makeDecode(Json.Decode.float), e => `FloatPoint(e))
  | `FloatCdf => j |> convert(FloatCdf.decode, e => `FloatCdf(e))
  | `Percentage =>
    j |> convert(Utils.makeDecode(Json.Decode.float), e => `Percentage(e))
  | `Binary => j |> convert(Utils.makeDecode(Json.Decode.bool), e => `Binary(e))
  | `UnresolvableResolution =>
    j
    |> convert(Utils.makeDecode(Json.Decode.string), e =>
