...
async function channelMemberships(root, args, context, info) {
  ...
  const compountId = { agentId, channelId };
  console.log('\x1b[36m ---> \x1b[0m Middleware (channelMemberships)', compountId);

  if (channelId && agentId) {
    const channelMembership = await data.channelMemberships.getOne(compountId);
